...
#In your console window, type code calculator.py. Note that you may have previously coded this file in 
# a previous lecture. In the text editor, make sure that your code appears as follows:
...
#Notice that you could plausibly test the above code on your own using some obvious numbers such as 2. 
# However, consider why you might want to create a test that ensures that the above code functions appropriately.

#Following convention, let’s create a new test program by typing code test_calculator.py and modify your code
# in the text editor as follows:
...
#Notice that we are importing the square function from square.py on the first line of code. 
# By convention, we are creating a function called test_square. Inside that function, 
# we define some conditions to test.

#In the console window, type python test_calculator.py. You’ll notice that nothing is being outputted. 
# It could be that everything is running fine! Alternatively, it could be that our test function did not 
# discover one of the “corner cases” that could produce an error.
...
#Python’s assert command allows us to tell the compiler that something, some assertion, is true. We can apply 
# this to our test code as follows:
...
#Notice that we are definitively asserting what square(2) and square(3) should equal. 
# Our code is reduced from four test lines down to two.
...
#Now running python test_square.py in the console window, you will notice that an AssertionError is 
# raised by the compiler. Essentially, this is the compiler telling us that one of our conditions was not met.
#One of the challenges that we are now facing is that our code could become even more burdensome if we wanted
# to provide more descriptive error output to our users. Plausibly, we could code as follows:
...
#Notice that running this code will produce multiple errors. However, it’s not producing all the errors above.
# This is a good illustration that it’s worth testing multiple cases such that you might catch situations where 
# there are coding mistakes.
...
#pytest allows us to run our program directly through it, such that we can more easily view the results of our test
#  conditions.
#In the terminal window, type pytest test_calculator.py. You’ll immediately notice that output will be provided.
#  Notice the red F near the top of the output, indicating that something in your code failed. Further, 
# notice that the red E provides some hints about the errors in your calculator.py program. Based upon the output, 
# you can imagine a scenario where 3 * 3 has outputted 6 instead of 9. Based on the results of this test, 
# we can go correct our calculator.py code as follows:
...
#At the moment, it is not ideal that pytest will stop running after the first failed test. 
# Again, let’s return our calculator.py code back to its broken state:
...
#In the terminal window, type pytest test_calculator.py. You’ll immediately notice that output will be provided. 
# Notice the red F near the top of the output, indicating that something in your code failed. Further, 
# notice that the red E provides some hints about the errors in your calculator.py program.
# Based upon the output, you can imagine a scenario where 3 * 3 has outputted 6 instead of 9. 
...
#Notice that we have divided the same five tests into three different functions. 
# Testing frameworks like pytest will run each function, even if there was a failure in one of them. 
# Re-running pytest test_calculator.py, you will notice that many more errors are being displayed. 
...
#Notice that we changed our hello function to return a string. 
#This effectively means that we can now use pytest to test the hello function.
...
#Notice that the above code separates our test into multiple functions such that they will all run, even 
...
#a special __init__ file. In your terminal window, create this file by typing code test/__init__.py. 
# Note the test/ as before, as well as the double underscores on either side of init. 
# Even leaving this __init__.py file empty, pytest is informed that the whole folder containing __init__.py has 
...
#Testing your code is a natural part of the programming process. Unit tests allow you to test specific aspects
#  of your code. You can create your own programs that test your code. Alternatively, you can utilize frameworks 
# like pytest to run your unit tests for you. In this lecture, you learned about…
